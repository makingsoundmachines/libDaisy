 ...
 */
void DaisySeedPercussive::Configure() {}

void DaisySeedPercussive::Init(bool boost)
{
    //dsy_system_init();
...
void DaisySeedPercussive::DeInit()
{
...
dsy_gpio_pin DaisySeedPercussive::GetPin(uint8_t pin_idx)
{
    dsy_gpio_pin p;
...
void DaisySeedPercussive::DelayMs(size_t del)
{
    system.Delay(del);
}

void DaisySeedPercussive::StartAudio(AudioHandle::InterleavingAudioCallback cb)
{
    audio_handle.Start(cb);
}

void DaisySeedPercussive::StartAudio(AudioHandle::AudioCallback cb)
{
    audio_handle.Start(cb);
}

void DaisySeedPercussive::ChangeAudioCallback(AudioHandle::InterleavingAudioCallback cb)
{
    audio_handle.ChangeCallback(cb);
}

void DaisySeedPercussive::ChangeAudioCallback(AudioHandle::AudioCallback cb)
{
    audio_handle.ChangeCallback(cb);
}

void DaisySeedPercussive::StopAudio()
{
    audio_handle.Stop();
}

void DaisySeedPercussive::SetAudioSampleRate(SaiHandle::Config::SampleRate samplerate)
{
    audio_handle.SetSampleRate(samplerate);
    callback_rate_ = AudioSampleRate() / AudioBlockSize();
}

float DaisySeedPercussive::AudioSampleRate()
{
    return audio_handle.GetSampleRate();
}

void DaisySeedPercussive::SetAudioBlockSize(size_t blocksize)
{
    audio_handle.SetBlockSize(blocksize);
    callback_rate_ = AudioSampleRate() / AudioBlockSize();
}

size_t DaisySeedPercussive::AudioBlockSize()
{
    return audio_handle.GetConfig().blocksize;
}

float DaisySeedPercussive::AudioCallbackRate() const
{
    return callback_rate_;
}

void DaisySeedPercussive::SetLed(bool state)
{
    dsy_gpio_write(&led, state);
}

void DaisySeedPercussive::SetTestPoint(bool state)
{
...
void DaisySeedPercussive::ConfigureQspi()
{
...
void DaisySeedPercussive::ConfigureAudio()
{
    // SAI1 -- Peripheral
    // Configure
    ...
    sai_config.a_dir         = SaiHandle::Config::Direction::TRANSMIT; // RECEIVE
    sai_config.pin_config.sa = {DSY_GPIOE, 6};
    sai_config.b_dir         = SaiHandle::Config::Direction::RECEIVE; // TRANSMIT
...
void DaisySeedPercussive::ConfigureDac()
{
...
DaisySeedPercussive::BoardVersion DaisySeedPercussive::CheckBoardVersion()
{
    /** Version Checks:
