...
static const std::map<Trill::Device, struct TrillDefaults> trillDefaults = {
    {Trill::NONE, TrillDefaults("No device", Trill::AUTO, 0, 0b11111111, -1)},                    // 0xFF
    {Trill::UNKNOWN, TrillDefaults("Unknown device", Trill::AUTO, 0, 0b11111111, -1)},          // 0xFF
    {Trill::BAR, TrillDefaults("Bar", Trill::CENTROID, defaultThreshold, 0b00100000, 2)},       // 0x20
    {Trill::SQUARE, TrillDefaults("Square", Trill::CENTROID, defaultThreshold, 0b00101000, 1)}, // 0x28
    {Trill::CRAFT, TrillDefaults("Craft", Trill::DIFF, defaultThreshold, 0b00110000, 1)},        // 0x30
    {Trill::RING, TrillDefaults("Ring", Trill::CENTROID, defaultThreshold, 0b00111000, 1)},     // 0x38
    {Trill::HEX, TrillDefaults("Hex", Trill::CENTROID, defaultThreshold, 0b01000000, 1)},       // 0x40
    {Trill::FLEX, TrillDefaults("Flex", Trill::CENTROID, 0.03, 0b01001000, 4)},                 // 0x48
};
...
int Trill::identify() {
    ...
    if((i2c_.TransmitBlocking(address, buf, bytesToWrite, timeout_)) != I2CHandle::Result::OK)
    {
        return -1;
    }
    preparedForDataRead_ = false;

    System::DelayUs(commandSleepTime); // need to give enough time to process command

    i2c_.ReceiveBlocking(address, dataBuffer.data(), 4, timeout_); // discard first read

    ssize_t bytesToRead = 4;
    if ((i2c_.ReceiveBlocking(address, dataBuffer.data(), bytesToRead, timeout_)) != I2CHandle::Result::OK)
    {
        fprintf(stderr, "Failure to read Byte Stream. Expected %d bytes\n" , bytesToRead);
        device_type_ = NONE;
        return -1;
    }
...
int Trill::setMinimumTouchSize(float minSize) {
    ...
    uint8_t buf[4] = { kOffsetCommand, kCommandMinimumSize, (uint8_t)(size >> 8), (uint8_t)(size & 0xFF) };
    if((i2c_.TransmitBlocking(address, buf, bytesToWrite, timeout_)) != I2CHandle::Result::OK)
    {
...
int Trill::setAutoScanInterval(uint16_t interval) {
    ssize_t bytesToWrite = 4;
    uint8_t buf[4] = { kOffsetCommand, kCommandAutoScanInterval, (uint8_t)(interval >> 8), (uint8_t)(interval & 0xFF) };
    if((i2c_.TransmitBlocking(address, buf, bytesToWrite, timeout_)) != I2CHandle::Result::OK)
    {
...
int Trill::readI2C() {
    ...
    {
        num_touches_ = 0;
        fprintf(stderr, "Trill: error while reading from device %s at address %#x (%d): Expected to read %d bytes (error: %d %s)\n",
            getNameFromDevice(device_type_).c_str(), address, address, bytesToRead, errno, strerror(errno));
