...
#include "daisy_stolperbeats_rev4.h"
...
void DaisyStolperbeatsRev4::Init(bool boost)
{
...
void DaisyStolperbeatsRev4::DelayMs(size_t del)
{
    seed.DelayMs(del);
}

void DaisyStolperbeatsRev4::SetHidUpdateRates()
{
...
void DaisyStolperbeatsRev4::StartAudio(AudioHandle::AudioCallback cb)
{
    seed.StartAudio(cb);
}

void DaisyStolperbeatsRev4::ChangeAudioCallback(AudioHandle::AudioCallback cb)
{
    seed.ChangeAudioCallback(cb);
}

void DaisyStolperbeatsRev4::StopAudio()
{
    seed.StopAudio();
}

void DaisyStolperbeatsRev4::SetAudioSampleRate(
...
float DaisyStolperbeatsRev4::AudioSampleRate()
{
    return seed.AudioSampleRate();
}

void DaisyStolperbeatsRev4::SetAudioBlockSize(size_t size)
{
    seed.SetAudioBlockSize(size);
    SetHidUpdateRates();
}

size_t DaisyStolperbeatsRev4::AudioBlockSize()
{
    return seed.AudioBlockSize();
}

float DaisyStolperbeatsRev4::AudioCallbackRate()
{
    return seed.AudioCallbackRate();
}

void DaisyStolperbeatsRev4::StartAdc()
{
    seed.adc.Start();
}

/** Stops Transfering data from the ADC */
void DaisyStolperbeatsRev4::StopAdc()
{
    seed.adc.Stop();
}

//ex DaisyStolperbeatsRev4::UpdateAnalogControls()
void DaisyStolperbeatsRev4::ProcessAnalogControls()
{
    for(size_t i = 0; i < CTRL_LAST; i++)
    {
        controls[i].Process();
...
//ex  DaisyStolperbeatsRev4::GetCtrlValue(Ctrl k)
float DaisyStolperbeatsRev4::GetKnobValue(Ctrl k)
{
    return (controls[k].Value());
}


//ex DaisyStolperbeatsRev4::DebounceControls()
void DaisyStolperbeatsRev4::ProcessDigitalControls()
{
...
void DaisyStolperbeatsRev4::InitTimer()
{
...
void DaisyStolperbeatsRev4::InitLEDMatrix()
{
...
void DaisyStolperbeatsRev4::InitLEDMatrixDMA()
{
...
void DaisyStolperbeatsRev4::InitAudio()
{
...
void DaisyStolperbeatsRev4::InitControls()
{
...
void DaisyStolperbeatsRev4::InitMidi()
{
    MidiUartHandler::Config midi_config;
    midi.Init(midi_config);
}

void DaisyStolperbeatsRev4::InitCvOutputs()
{
...
void DaisyStolperbeatsRev4::InitEncoder()
{
    ...
    encoder[ENC_2].Init({DSY_GPIOA, 10}, {DSY_GPIOB,  4},  {DSY_GPIOE,  1}, AudioCallbackRate()); // ENCODER KICK
    ...
    encoder[ENC_6].Init({DSY_GPIOD,  0}, {DSY_GPIOE,  0},  {DSY_GPIOX,  0}, AudioCallbackRate()); // ENCODER SHUFFLE - Click is dummy
    encoder[ENC_7].Init({DSY_GPIOC,  2}, {DSY_GPIOD,  1},  {DSY_GPIOD,  9}, AudioCallbackRate()); // ENCODER PERC 1
    encoder[ENC_8].Init({DSY_GPIOC,  7}, {DSY_GPIOD,  7},  {DSY_GPIOD, 10}, AudioCallbackRate()); // ENCODER PERC 2
    encoder[ENC_9].Init({DSY_GPIOE,  9}, {DSY_GPIOE, 11},  {DSY_GPIOD,  6}, AudioCallbackRate()); // ENCODER SUBDIV
    encoder[ENC_10].Init({DSY_GPIOE, 10}, {DSY_GPIOE, 12},  {DSY_GPIOE, 8}, AudioCallbackRate()); // ENCODER SYNC
}

void DaisyStolperbeatsRev4::InitSR595()
{
    num_sr_chained = 5;

    sr_pin_cfg[SR_LATCH] = {DSY_GPIOD, 3}; //seed.GetPin(PIN_SR_LATCH);
    sr_pin_cfg[SR_CLOCK] = {DSY_GPIOD, 4}; //seed.GetPin(PIN_SR_CLOCK);
    sr_pin_cfg[SR_DATA]  = {DSY_GPIOD, 5}; //seed.GetPin(PIN_SR_DATA);
...
{
    return sr_4021_state_[idx] == 0x0000;
}

bool DaisyStolperbeatsRev4::SR_4021RisingEdge(size_t idx) const
{
    return sr_4021_state_[idx] == 0x8000;
}

bool DaisyStolperbeatsRev4::SR_4021FallingEdge(size_t idx) const
{
    return sr_4021_state_[idx] == 0x7fff;
}


void DaisyStolperbeatsRev4::InitButtons()
{
    // Buttons
    // dsy_gpio_pin pin;
...
void DaisyStolperbeatsRev4::InitGates()
{
    // Gate Output
    ...
    gate_output[GATE_OUT_4].pin  = {DSY_GPIOB, 2}; // TRIGGER OUT HIHAT 2
    ...
    gate_output[GATE_OUT_5].pin = {DSY_GPIOA, 8}; // TRIGGER OUT PERC 1
    ...
    dsy_gpio_pin pin;
    pin = {DSY_GPIOC, 14}; gate_input[GATE_IN_1].Init(&pin);  // TAP
    pin = {DSY_GPIOB, 10}; gate_input[GATE_IN_2].Init(&pin);  // RESET (LINEAR DRUMMING) 
